...
class TitanFrameworkOptionUpload extends TitanFrameworkOption {
	...
	// For live previews, we need to give the uploaded URL to javascript
	...
	public static function createUploaderScript() {
		...
		jQuery(document).ready(function($){
			...
			$('body').on('click', '.tf-upload .thumbnail, .tf-upload img', function(event) {
				...
				var frame = wp.media({
					title: '<?php echo esc_html__( 'Select Image', 'laread' ) ?>',
					multiple: false,
					library: { type: 'image' },
					button : { text : '<?php echo esc_html__( 'Use image', 'laread' ) ?>' }
				});
