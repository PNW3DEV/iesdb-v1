	...
	class TitanFrameworkOptionEddLicense extends TitanFrameworkOption {
		...
		public function display() {
			...
			if ( strlen( $license ) > 0 ) {
				...
				switch( $status ) {
					...
					case 'valid':
						?><p class="description"><?php echo esc_html__( 'Your license is valid and active.', 'laread' ); ?></p><?php
					break;

					case 'invalid':
						?><p class="description"><?php echo esc_html__( 'Your license is invalid.', 'laread' ); ?></p><?php
					break;
					...
					case 'inactive':
						...
						?>
						<a href="<?php echo $url; ?>" class="button-secondary"><?php echo esc_html__( 'Activate', 'laread' ); ?></a>
						<p class="description"><?php echo esc_html__( 'Your license is valid but inactive. Click the button above to activate it.', 'laread' ); ?></p><?php

					break;

					case 'no_response':
						?><p class="description"><?php echo esc_html__( 'The remote server did not return a valid response. You can retry by hitting the &laquo;Save&raquo; button again.', 'laread' ); ?></p><?php
					break;

				}

			} else {
				?><p class="description"><?php echo esc_html__( 'Entering your license key is mandatory to get the product updates.', 'laread' ); ?></p><?php
			}
		...
		public function checkUpdates() {
			...
			elseif ( in_array( $item_is, array( 'theme-parent', 'theme-child' ) ) ) {
				...
				$strings = array(
					'theme-license'             => __( 'Theme License', 'laread' ),
					'enter-key'                 => __( 'Enter your theme license key.', 'laread' ),
					'license-key'               => __( 'License Key', 'laread' ),
					'license-action'            => __( 'License Action', 'laread' ),
					'deactivate-license'        => __( 'Deactivate License', 'laread' ),
					'activate-license'          => __( 'Activate License', 'laread' ),
					'status-unknown'            => __( 'License status is unknown.', 'laread' ),
					'renew'                     => __( 'Renew?', 'laread' ),
					'unlimited'                 => __( 'unlimited', 'laread' ),
					'license-key-is-active'     => __( 'License key is active.', 'laread' ),
					'expires%s'                 => __( 'Expires %s.', 'laread' ),
					'%1$s/%2$-sites'            => __( 'You have %1$s / %2$s sites activated.', 'laread' ),
					'license-key-expired-%s'    => __( 'License key expired %s.', 'laread' ),
					'license-key-expired'       => __( 'License key has expired.', 'laread' ),
					'license-keys-do-not-match' => __( 'License keys do not match.', 'laread' ),
					'license-is-inactive'       => __( 'License is inactive.', 'laread' ),
					'license-key-is-disabled'   => __( 'License key is disabled.', 'laread' ),
					'site-is-inactive'          => __( 'Site is inactive.', 'laread' ),
					'license-status-unknown'    => __( 'License status is unknown.', 'laread' ),
					'update-notice'             => __( "Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'laread' ),
					'update-available'          => __('<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'laread' )
				);
