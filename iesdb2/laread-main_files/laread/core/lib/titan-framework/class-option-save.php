...
class TitanFrameworkOptionSave extends TitanFrameworkOption {
	...
	public function display() {
		...
		if ( empty( $this->settings['save'] ) ) {
			$this->settings['save'] = __( 'Save Changes', 'laread' );
		}
		if ( empty( $this->settings['reset'] ) ) {
			$this->settings['reset'] = __( 'Reset to Defaults', 'laread' );
		}
		if ( empty( $this->settings['reset_question'] ) ) {
			$this->settings['reset_question'] = __( 'Are you sure you want to reset ALL options to their default values?', 'laread' );
		}
