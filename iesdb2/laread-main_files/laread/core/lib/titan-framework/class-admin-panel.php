...
class TitanFrameworkAdminPanel {
	...
	public function register() {
		// Parent menu
		if ( empty( $this->settings['parent'] ) ) {
			$this->panelID = add_theme_page( $this->settings['name'],
						   $this->settings['name'],
		...
		// Sub menu
			...
			$this->panelID = add_theme_page( $this->settings['parent'],
							  $this->settings['name'],
							  $this->settings['name'],
							  $this->settings['capability'],
							  $this->settings['id'],
	...
	public function addTitanCreditText() {
		echo __( "<em>Options Page Created with <a href='http://titanframework.net?utm_source=admin&utm_medium=admin footer'>Titan Framework</a></em>", 'laread' );
	}
	...
	public function createAdminPage() {
		...
		if ( ! empty( $_GET['message'] ) ) {
			if ( $_GET['message'] === 'saved' ) {
				echo TitanFrameworkAdminNotification::formNotification( __( 'Settings saved.', 'laread' ), esc_html( $_GET['message'] ) );
			} else if ( $_GET['message'] === 'reset' ) {
				echo TitanFrameworkAdminNotification::formNotification( __( 'Settings reset to default.', 'laread' ), esc_html( $_GET['message'] ) );
			}
		}
